...
impl App {
    ...
    unsafe fn write_to_tag(&mut self) {
        ...
        match proto.write_image(data.as_ref()) {
            ...
            Err(e) => {
                let msg = match e {
                    protocol::NfcError::Busy => c_str!("NFC busy"),
                    protocol::NfcError::NoTag => c_str!("No tag found"),
                    protocol::NfcError::DetectFailed => c_str!("Detection failed"),
                    protocol::NfcError::TransmitFailed => c_str!("Transmit failed"),
                    protocol::NfcError::BadResponse => c_str!("Bad response"),
                    protocol::NfcError::Timeout => c_str!("Timeout"),
